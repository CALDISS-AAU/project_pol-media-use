...
def get_links_eb(pagesoup, page_params, keywords = [r".*"]):
    section_soup = pagesoup.find('div', class_ = 'section-list grid') 
...
def get_article_info(link, keywords, source, source_url, get_source = False):
    ...
    while i > 0:
        ...
        if response_code == 200:     
            ...
            try:
                info['article_paywall'], info['article_text'] = textdl.get_arttext(info.get('article_link'), driver_path = driver_path, source = source, 
